...
function FSendMailMarketingHunghacompanyTimviec365(title,content,receiver){
    return new Promise((resolve,reject)=>{
        let transporter = nodemailer.createTransport({
            service:'gmail',
            auth:{
            user:'nhatuyendungtimviec.hhp@gmail.com',
            pass:'zspqupawgaljzyue'
            }
        });
        const mail_config = {
            from:'nhatuyendungtimviec.hhp@gmail.com',
            to:receiver,
            subject:title,
            html: `${content}`
        };
...
function FSendMailMarketingHunghacompanyTimviec365_v2(title,content,receiver){
    return new Promise((resolve,reject)=>{
        let transporter = nodemailer.createTransport({
            service:'gmail',
            auth:{
            user:'uvtimviec365.hhp@gmail.com',
            pass:'hjwcotiaemsmwbdh'
            }
        });
        const mail_config = {
            from:'uvtimviec365.hhp@gmail.com',
            to:receiver,
            subject:title,
            html: `${content}`
        };
...
app.post('/api/mail/nhatuyendungtimviec_v2', SendMailMarketingHunghacompanyTimviec365_v2);
app.post('/api/mail/SendMailMarketingHunghacompanyTimviec365_v3', SendMailMarketingHunghacompanyTimviec365_v3);
app.listen(8800,()=>{
